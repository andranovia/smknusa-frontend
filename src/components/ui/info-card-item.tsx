        ...
        <div className="flex justify-start xl:grid grid-cols-2   items-center gap-1 sm:gap-2 top-2 lg:top-0 left-0 absolute lg:relative xl:absolute px-1.5 lg:px-0 xl:p-2 z-20">
          {infoCardData.level === "0" || infoCardData.level === 0 ? (
            <div className={`bg-[#FFE7AF] px-2 py-1 rounded-[6px] xl:rounded-[10px] max-w-[89.25px]`}>
              <p className="font-[500] text-[8px] md:text-[10px] text-center text-gray">Penting</p>
            </div>
          ) : null}

          <div style={{ backgroundColor: infoCardData.category.color }} className={cn(` px-2 py-1 rounded-[6px] xl:rounded-[10px] max-w-[74px] sm:max-w-[89.25px] `)}>
            <Heading type="h5" className="!font-[500] text-center text-[8px] md:!text-[10px] line-clamp-1   text-ellipsis  text-gray">
              {infoCardData.category.nama}
            </Heading>
          </div>
        </div>
        
          <Heading type="h5" className="text-sm md:text-[16px]  !font-[500] min-h-16 lg:min-h-fit xl:min-h-fit  xl:text-lg mb-2 sm:w-full  line-clamp-3 xl:!leading-6"> {infoCardData.nama}</Heading>
