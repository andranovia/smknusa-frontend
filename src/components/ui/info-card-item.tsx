...
const InfoCardItem = ({ infoCardData, normalDate }: InfoCardItemProps) => {
  return (
    <div className="bg-white rounded-lg w-full 1xl:w-[23rem] h-full shadow-md overflow-hidden relative">
      <Image
        className="w-full min-h-[8rem] sm:min-h-[8rem] xl:min-h-[12rem]  max-h-[6rem] sm:max-h-[10rem] lg:max-h-[8rem] xl:max-h-[12rem] object-cover"
        src={backendUrl + infoCardData.thumbnail}
        alt={infoCardData.nama}
        width={800}
        height={800}
      />
      <div className="px-2 sm:px-3 xl:p-4 flex flex-col items-stretch justify-between lg:h-[11.6875rem] xl:h-[10rem]   gap-4 w-full my-4 xl:my-0 ">
        <div className="flex justify-start xl:grid grid-cols-2   items-center gap-1 sm:gap-2 top-2 lg:top-0 left-0 absolute lg:relative xl:absolute px-1.5 lg:px-0 xl:p-2 z-20">
          ...
          <Heading type="h5" className="xl:text-md text-xs font-[500] min-h-16 lg:min-h-fit xl:min-h-fit  xl:text-lg mb-2 sm:w-full  line-clamp-3 !leading-6"> {infoCardData.nama}</Heading>
     
        <div className="text-sm gap-2 text-gray flex flex-row lg:flex-col xl:flex-row justify-between xl:items-center w-full">
