...
export async function generateStaticParams() {
    ...
    const ids = extracurricularData?.map((extracurricular: Extra) => extracurricular.id_extra);
...
async function getExtacurricularById(id: string) {
    ...
    const response = await fetch(`${backendUrl}api/user/profile/ekstras/${id}`, { cache: 'no-store' });
    ...
    return (
        <DetailLayout detailData={extracurricularById} className='justify-start'>
            <div className='flex flex-col gap-[3.5rem] xl:min-h-[20rem] w-full lg:w-fit'>
                <div className="flex flex-col lg:flex-row justify-center items-center gap-4 h-full w-full">
                ...
                <Paragraph>
                    {extracurricularById?.extra_text}
                </Paragraph>
                            ...
                            .map((extracurricular, index) => {
                                return (
                                    <React.Fragment key={index}>
                                        <Link href={`/academic/extracurricular/${extracurricular.id_extra}`}>
