...
const NavigationMenuItem = ({ name, currentDropdown, setCurrentDropdown, setShowMenu }: { name: string, currentDropdown: string | null, setCurrentDropdown: (key: string | null) => void, setShowMenu: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    return (
        <motion.div initial={false} animate={{height: isOpen ? name === 'BKK'?  '11rem' : '20rem' : '4rem'}} transition={defaultTransition} className={`flex flex-col gap-4 my-3 relative overflow-hidden ${name === 'BKK' ? 'lg:col-span-2' : ''}`}>
            <div className='flex justify-between items-center' onClick={() => handleToggleDropdown()}>
...
const NavigationHamburger = ({  showMenu, setShowMenu }: { showMenu: boolean, setShowMenu: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    return (
            ...
            <motion.div initial={{ x: isMobile ? 600 : 1050}} animate={{ x: showMenu ? 0 : isMobile ? 600 : 1050 }} transition={{...defaultTransition, type: 'tween'}} className={` fixed top-0 left-0 w-full min-h-screen bg-primary z-40 flex flex-col gap-4 ${showMenu ? 'pointer-events-auto' : 'pointer-events-none'}`}>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 lg:gap-4 px-[34px]'>
                    <NavigationMenuItem name="Akademik" currentDropdown={currentDropdown} setCurrentDropdown={setCurrentDropdown} setShowMenu={setShowMenu}/>
                    <NavigationMenuItem name="Profile" currentDropdown={currentDropdown} setCurrentDropdown={setCurrentDropdown} setShowMenu={setShowMenu} />
                    <NavigationMenuItem name="BKK" currentDropdown={currentDropdown} setCurrentDropdown={setCurrentDropdown}  setShowMenu={setShowMenu}/>
                    <Link href="/news" onClick={() => setShowMenu(false)} className=' my-3 lg:col-span-2'>  <Heading type="h5" className='text-white clas'>
                        Berita
                    </Heading></Link>
                    <hr className='border border-white my-5 lg:col-span-2' />
                    <Link href="/article" onClick={() => setShowMenu(false)} className=' my-5 lg:col-span-2'>  <Heading type="h5" className='text-white'>
                        Artikel
                    </Heading></Link>
